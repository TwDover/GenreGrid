...
- **Version history** — every re-roll/add snapshots the song first; the History picker restores any of the last 50 states (and a restore is itself restorable)
- **Portable projects (`.ggproj`)** — export a whole song session (every stem + structure) to one `.ggproj` file to archive it or open it on another machine; **Import** in the Songs rail restores it faithfully into a fresh song
...
- **Audio export (WAV / MP3 / OGG)** — offline-render the full mix or true per-part stems, matching the preview voices; a **WAV / MP3 / OGG** toggle on the Export row picks the format (WAV lossless, MP3/OGG compressed for sharing — encoded in-app, no upload), and every part card and the whole song offer a one-click **↓** download alongside **↓ .mid**
